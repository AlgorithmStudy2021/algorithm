...
public class Main {
    ...
    public int solution(String name) {
        ...
        int answer = 0;
        StringBuilder nameBuilder = new StringBuilder(name);//만들어야할 결과값 문자열
        StringBuilder aBuilder = new StringBuilder("");//A문자열 초기화 된것
        //A문자열 생성
        ...
        while(true) {
            ...
            if (nameBuilder.charAt(start) != 'A') {
                answer += getMin(nameBuilder.charAt(start));//위,아래중 더 적은 이동을 찾는다. input 해당 인덱스의 문자열 output 거리
                nameBuilder.setCharAt(start,'A');//name의 문자를 A로 바꿔준다. 주어진 인덱스의 문자를 a로 변경
            }
