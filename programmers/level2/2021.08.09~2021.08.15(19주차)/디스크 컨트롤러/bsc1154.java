    public static int solution(int[][] jobs) {

        int answer = 0;
        int end = 0; // 수행되고난 직후의 시간
        int jobsIdx = 0; // jobs 배열의 인덱스
        int count = 0; // 수행된 요청 갯수

        // 원본 배열 오름차순 정렬 (요청시간 오름차순)
        Arrays.sort(jobs, (o1, o2) -> o1[0] - o2[0]);

        // 처리 시간 오름차순으로 정렬되는 우선순위 큐(Heap)
        PriorityQueue<int[]> pq = new PriorityQueue<>((o1, o2) -> o1[1] - o2[1]);

        // 요청이 모두 수행될 때까지 반복
        while (count < jobs.length) {

            // 하나의 작업이 완료되는 시점(end)까지 들어온 모든 요청을 큐에 넣음
            //[[0, 3], [1, 9], [2, 6]]
            while (jobsIdx < jobs.length && jobs[jobsIdx][0] <= end) {//작업개수 이하이면서 && 시작시점이 수행되고난 직후의 시간 이하일경우 어떤 요청도 들어올수있음
                pq.add(jobs[jobsIdx++]);
            }

            // 큐가 비어있다면 작업 완료(end) 이후에 다시 요청이 들어온다는 의미
            // (end를 요청의 가장 처음으로 맞춰줌)
            if (pq.isEmpty()) {
                end = jobs[jobsIdx][0];
                // 작업이 끝나기 전(end 이전) 들어온 요청 중 가장 수행시간이 짧은 요청부터 수행
            } else {
                System.out.println("=====================");
                int[] temp = pq.poll();//가장 윗수를 꺼내서 저장
                System.out.println(temp);
                answer += temp[1] + end - temp[0];//처리시간 + 현재시간 - 요청시간
                System.out.println("temp[1] : "+temp[1]);
                System.out.println("end : "+end);
                System.out.println("temp[0] : "+temp[0]);
                end += temp[1];//현재 시간에는 처리시간을 더해주고
                System.out.println(end);
                count++;
            }
        }

        return (int) Math.floor(answer / jobs.length);
    }
